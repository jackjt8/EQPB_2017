...
def method2_plot(satlist):
    for this_sat in satlist:        
        ...
        ch0mean2 = ch0mean[ch0mean < 30000]
        peakmean2 = peakmean[ch0mean < 30000]
        ourmpldates2 = ourmpldates[ch0mean < 30000]
        
        #%%
        
        #fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(20,10), dpi=80)
        
        fig = plt.figure(figsize=(20, 15), dpi=80)
        gs1 = gridspec.GridSpec(4, 2) #7,6
        gs1.update(wspace=0.15, hspace=0.15)
        fig.tight_layout()
    

        titletext = 'method2 plot SVN%s \n Velocity=%2.2fkm/s ; Period=%2.2f minutes' % (this_sat, np.nanmean(vel), np.nanmean(periodm) )
        #print titletext
        plt.suptitle(titletext, fontsize=20)
        ...
        for i in range(int(yearu[0]),int(yearu[len(yearu)-1]+2)):
            datetime(i,1,1,0,0,0)
            ax1.axvline(x=datetime(i,1,1,0,0,0), color='grey',lw=.5)     
        ax1.grid()
        
        L1, = ax1.plot_date(ourmpldates2,ch0mean2, label='ch0 mean')
        L2, = ax1.plot_date(ourmpldates2,peakmean2, label='peak ch0 mean')
        
        #ax1.set_title('Ch0 mean and peak mean')
        ax1.set_ylabel('Ch0 count rate (Hz)', fontsize = 10)
        ...
        for i in range(int(yearu[0]),int(yearu[len(yearu)-1]+2)):
            datetime(i,1,1,0,0,0)
            ax2.axvline(x=datetime(i,1,1,0,0,0), color='grey',lw=.5)     
        ax2.grid()
        
        L3, = ax2.plot_date(ourmpldates,pdw, label='peak dday width')
        ...
        for i in range(int(yearu[0]),int(yearu[len(yearu)-1]+2)):
            datetime(i,1,1,0,0,0)
            ax3.axvline(x=datetime(i,1,1,0,0,0), color='grey',lw=.5)       
        ax3.grid()
        
        L3, = ax3.plot_date(ourmpldates, pawmin, label='peak angle width min')
        ...
        for i in range(int(yearu[0]),int(yearu[len(yearu)-1]+2)):
            datetime(i,1,1,0,0,0)
            ax4.axvline(x=datetime(i,1,1,0,0,0), color='grey',lw=.5)       
        ax4.grid()
        
        L4, = ax4.plot_date(ourmpldates, paww, label='peak angle width width')
        ax4.axhline(y=np.nanmean(paww), label='Mean = %.2f' %(np.nanmean(paww)), color='red' )
        ax4.axhline(y=np.nanmean(paww)+np.nanstd(paww), label='Standard Deviation = %.2f' %(np.nanstd(paww)), color='green' )
        ax4.axhline(y=np.nanmean(paww)-np.nanstd(paww), color='green' )

        ax4.set_ylabel('Angular width in degrees', fontsize = 10)
        ...
        #%%
        # Force x-axis...
        ax1.set_xlim(datetime(2001,1,1,0,0,0), datetime(2017,1,1,0,0,0))
        ...
        stemp = "method2_" + str(this_sat) + "_plot.png"
        plt.savefig(stemp,bbox_inches="tight")
        fig.clear() #cleanup
        plt.clf() #cleanup
        plt.cla() #cleanup
